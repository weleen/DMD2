...
def parse_args():
    ...
    parser.add_argument(
        "--death_mode",
        type=str,
        choices=["uniform", "linear", "none"],
        help="The mode of stochastic depth. Choose between 'uniform', 'linear' or 'none'.",
    )
    parser.add_argument(
        "--death_rate",
        type=float,
        default=0.5,
        help="The rate of stochastic depth. Default is 0.5.",
    )
    parser.add_argument(
        "--replace_strategy",
        type=str,
        choices=["interpolation", "conv"],
        help="The strategy to replace the skipped layers. Choose between 'interpolation' or 'conv'.",
    )
    parser.add_argument(
        "--enable_teacher_distillation",
        action="store_true",
        help="Enable teacher distillation for the robust training.",
    )
    parser.add_argument("--unet_config_path", type=str, default="main/robust_training/unet_config/config.json")
...
def main():
    ...
    ) if args.enable_teacher_distillation else None
    # BUG: unet.config is not matched with unet_teacher.config, save checkpoint error!
    unet = UNet2DConditionalModelStochasticDepth.from_pretrained(
        pretrained_model_name_or_path=args.pretrained_model_name_or_path, revision=args.non_ema_revision, config_file=args.unet_config_path, subfolder='unet', death_mode=args.death_mode, death_rate=args.death_rate, replace_strategy=args.replace_strategy, is_debug=args.is_debug)
    # unet = UNet2DConditionalModelStochasticDepth.from_pretrained(
    #     args.pretrained_model_name_or_path, subfolder="unet", revision=args.non_ema_revision,
    #     death_mode=args.death_mode, death_rate=args.death_rate, replace_strategy=args.replace_strategy, is_debug=args.is_debug
    # )
    if args.is_debug:
    ...
    if args.use_ema:
        ema_unet = UNet2DConditionalModelStochasticDepth.from_config(pretrained_model_name_or_path=args.pretrained_model_name_or_path, revision=args.non_ema_revision, config_file=args.unet_config_path, subfolder='unet')
        ema_unet = EMAModel(
            ema_unet.parameters(),
            model_cls=UNet2DConditionalModelStochasticDepth,
    ...
    if args.enable_teacher_distillation:
    ...
    # Add hook for feature distillation
    if args.enable_teacher_distillation:
        acts_tea = {}
        acts_stu = {}
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(unet):
                ...
                if args.snr_gamma is None:
                    ...
                    loss_sd = F.mse_loss(model_pred.float(), target.float(), reduction="none")
                ...
                if args.enable_teacher_distillation:
                    ...
                    loss_kd_output = torch.zeros_like(loss_sd)
                    loss_kd_feat = torch.zeros_like(loss_sd)
